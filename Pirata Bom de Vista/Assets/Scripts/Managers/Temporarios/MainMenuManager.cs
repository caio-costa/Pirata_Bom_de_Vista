...
public class MainMenuManager : MonoBehaviour {
    ...
    public Color corSelecionado, corOriginal, corTextos;
    public Material oceano;
    public GameObject menuConfirmacao;
    public Transform pivotLentes;
    public float velocidadeRotacao = 0;
    ...
    public void EsperarTerminoFade() {
    ...
    private void SelecionaAtual()
    {
        if (inputPermitido) {
            ...
            StopAllCoroutines();
            StartCoroutine(RotacionaLentes(itemAtual));
        }
    ...
    public IEnumerator RotacionaLentes(int i) {
        float target = i * 90;
        //Debug.Log("Target: " + target);
        float angle = pivotLentes.eulerAngles.z;

        while (angle != target) {
            angle = Mathf.MoveTowardsAngle(pivotLentes.eulerAngles.z, target, velocidadeRotacao * Time.deltaTime);
            pivotLentes.eulerAngles = new Vector3(0, 180, angle);
    ...
    public void AtivaBotao() {
        itens[itemAtual].GetComponent<ItemdeMenu>().AtivaBotao();
    }
